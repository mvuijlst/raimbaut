"""
Build manifest_v2.json: the reading order of the 2026 edition.
...
page number (from page_numbers_v2.csv; blank for unnumbered leaves) for anchors,
and a coarse `kind` from the leading section header.

Output: manifest_v2.json -- ordered [{order, pageid, vol, idx, printed, file, kind}].
"""
import csv
import glob
import json
...
PRINTED = {r["pageid"]: r["printed_number"]
           for r in csv.DictReader(open("page_numbers_v2.csv", encoding="utf-8"))}
...
manifest = []
for order, f in enumerate(sorted(glob.glob("corpus-v2/*.md"))):
...
json.dump(manifest, open("manifest_v2.json", "w", encoding="utf-8"),
          ensure_ascii=False, indent=2)
from collections import Counter
c = Counter(m["kind"] for m in manifest)
print(f"Wrote {len(manifest)} pages -> manifest_v2.json")
print("kinds:", dict(c))
